...
extension BasicBlock {
    ...
    mutating func localValueNumbering(showingDebugInfo: Bool = false) {
        ...
        for index in self.instrs.indices {
            ...
            case .Instruction(let instr):
                ...
                // TODO: Fix all of this. It's a mess.
                /*
                 In the case of the test case `nonlocal`:
                 From this:
                 @main {
                   a: int = const 4;
                   b: int = const 2;
                 ...
                 BUT we should constant propagate the arguments of the prod instruction.
                 This doesn't happen because sum1 and sum2 are defined outside the block, so
                 in LVN.swift:450, when we do:
                    if let value = lvn.valueFrom(canonicalHome: canonicalHomeForArgOfIdInstr) {
                      if value.op == "const" {
                        ...
                 That check if value.op == "const" gives False! Because value.op == "OutsideBlock".
                 So, an obvious first solution would be to give the op "OutsideBlock" some context
                 of it's a const or not, but this is an awful solution.
                 I'm sure there is a better way for dealing with values defined outside of the block.
                 So, we need to rethink the whole implementation.
                
                Here, the upcoming if here is nonsense. TODO: FIX THIS!
                */
                if !valueArgsDefinedOutsideOfBlock.isEmpty {
                ...
                if var varAssignedTo = instr.dest {
                    var indexInTableForVar: IndexIntoTable? = nil
        ...
        // PARA EL TEST CASE nonlocal ESTO ROMPEEEEE
        self.constantFolding(&lvn)
    }
    
    
    // To be run *after* localValueNumbering() (Assumes the lvnContext is already built).
    ...
    */
    // Given an instruction, returns true if all of its arguments are constants.
    func areInstructionArgumentsContants(_ instr: Instruction, _ lvn: LVNContext) -> Bool {
        if instr.args == nil {
            return false
        }
        let constants = constantsOfInstruction(instr, lvn)
        return constants.count == instr.args!.count
    }
    ...
    mutating func constantFoldingArithmeticInstruction(indexOfInstr: Index, arithmeticOp: (Int, Int) -> Int, _ lvn: inout LVNContext) -> Bool {
        ...
        switch instr_ {
            ...
            case .Instruction(let instr):
                ...
                let resultOfApplyingArithmeticOpToConstants: Int = arithmeticOp(constantArgs[0], constantArgs[1])
                self.replaceInstr(at: indexOfInstr, toConstValue: IntOrBoolean.int(resultOfApplyingArithmeticOpToConstants))
                // Not only replace the actual instruction in the block, but also update the LVN table:
                lvn.updateInstruction(atIndexInTable: indexOfInstr, withNewValue: Value(op: "const", args: [resultOfApplyingArithmeticOpToConstants]))
                return true
        }
        return false
    }
    ...
    // {gt, lt} X X => false
    // {ge, le} X X => true
    mutating func constantFoldingComparisonInstruction(indexOfInstr: Index, comparisonOp: String, _ lvn: inout LVNContext) -> Bool {
    ...
    mutating func replaceInstr(at indexOfInstruction: Index, toConstValue: IntOrBoolean) {
    ...
    mutating func replaceInstr(at indexOfInstruction: Index, withCopyOfVar: CanonicalHomeForVariable) {
        let instr_ = self.instrs[indexOfInstruction]
        switch instr_ {
        case .Label(_):
            break
    ...
    mutating func replaceArgument(_ oldArg: String, ofInstructionAt indexOfInstruction: Index, withNewArgument newArg: String) {
        let instr_ = self.instrs[indexOfInstruction]
        switch instr_ {
        case .Label(_):
            break
    ...
    func isValidInstructionIndex(_ index: Index) -> Bool {
    ...
    func getIndexOfInstructionWhereVariableIsDefined(_ varName: String, _ lvn: inout LVNContext) -> Index? {
        assert(self.isValidVariableName(varName, &lvn))
        
        var indexToVarDef: Index? = nil
    ...
    func isVariableEverOverwritten(varName: String, betweenStartIndex startIndex: Index, andUntilIndex untilIndex: Index, _ lvn: inout LVNContext) -> (Bool, Index?) {
        assert(self.isValidInstructionIndex(startIndex))
        assert(self.isValidInstructionIndex(untilIndex))
        assert(self.isValidVariableName(varName, &lvn))
        
        var isOverwritten: Bool = false
        var indexOfInstructionOverwriter: Index? = nil
    ...
    func isInstructionOverwrittenLater2(at indexOfInstruction: Index, _ lvn: inout LVNContext) -> (Bool, Int?) {
    ...
    func isInstructionOverwrittenLater(at indexOfInstruction: Index) -> (Bool, Index?) {
        assert(self.isValidInstructionIndex(indexOfInstruction))
    ...
    mutating func replaceUsesOfVar(_ varToReplace: String, withNewVar newVar: String, untilIndex upperBoundIndex: Index) {
        for index in self.instrs.indices {
    ...
    mutating func replaceNameOfVarAssignedTo(ofInstructionAt indexOfInstruction: Index) -> String {
        let instr_ = self.instrs[indexOfInstruction]
        switch instr_ {
        case .Label(_):
            break
...
extension Function {
    ...
    mutating func localValueNumbering(showingDebugInfo: Bool = false) {
        for indexBB in self.basicBlocks.indices {
            ...
            self.basicBlocks[indexBB].localValueNumbering(showingDebugInfo: showingDebugInfo)
        }
    }
